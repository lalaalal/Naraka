...
public class HerobrineEyeLayer<T extends AbstractHerobrine> extends RenderLayer<T, AbstractHerobrineModel<T>> {
    public HerobrineEyeLayer(RenderLayerParent<T, AbstractHerobrineModel<T>> renderer) {
        super(renderer);
    }
    ...
    public void render(PoseStack poseStack, MultiBufferSource buffer, int packedLight, T herobrine, float limbSwing, float limbSwingAmount, float partialTick, float ageInTicks, float netHeadYaw, float headPitch) {
        ...
        AbstractHerobrineModel<T> model = getParentModel();

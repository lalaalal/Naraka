...
public class BeamEffectLayer<T extends Entity, M extends EntityModel<T>> extends RenderLayer<T, M> {
    ...
    public static List<BeamEffectRenderState> createRenderStates(List<BeamEffect> beamEffects, float ageInTicks) {
        List<BeamEffectRenderState> renderStates = new ArrayList<>();
        for (BeamEffect beamEffect : beamEffects) {
            BeamEffectRenderState beamEffectRenderState = new BeamEffectRenderState(beamEffect);
            renderStates.add(beamEffectRenderState);
    ...
    public void render(PoseStack poseStack, MultiBufferSource bufferSource, int packedLight, T livingEntity, float limbSwing, float limbSwingAmount, float partialTick, float ageInTicks, float netHeadYaw, float headPitch) {
        ...
        for (BeamEffectRenderState beamEffectRenderState : createRenderStates(BeamEffectsHelper.get(livingEntity), ageInTicks)) {

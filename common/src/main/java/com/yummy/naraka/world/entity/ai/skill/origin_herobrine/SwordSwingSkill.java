...
public class SwordSwingSkill extends AttackSkill<OriginHerobrine> {
    public static final ResourceLocation LOCATION = createLocation("origin_herobrine.sword_swing");
    ...
    protected void onFirstTick(ServerLevel level) {
        Vec3 position = NarakaUtils.horizontalVec3(mob.getLookAngle())
                .scale(10)
    ...
    protected void tickAlways(ServerLevel level, @Nullable LivingEntity target) {
        if (narakaSword != null) {
            ...
            runAt(110, () -> {
                ...
                NetworkManager.clientbound().send(mob.players(), packet);
                NarakaSkillUtils.shockwaveBlocks(level, mob.players(), mob.blockPosition(), 16,
                        NarakaUtils.CIRCLE,
    ...
    private void floatLine(ServerLevel level, int radius, int waveTick, Supplier<Vec3> movement) {
        ...
        NarakaSkillUtils.shockwaveBlocks(level, mob.players(), mob.blockPosition(), radius + waveTick, predicate, movement);
    }
    ...
    private ResourceLocation getMotionIdentifier() {
        if (mob.getCurrentAnimation().equals(HerobrineAnimationLocations.SWORD_ATTACK_SPIN))

 ...
 * @see StigmaHelper
 */
public record Stigma(int value, long lastMarkedTime, Optional<UUID> cause) {
    public static final Codec<Stigma> CODEC = RecordCodecBuilder.create(instance -> instance.group(
            ...
            UUIDUtil.CODEC.optionalFieldOf("cause").forGetter(Stigma::cause)
    ...
    public Stigma(int value, long lastMarkedTime, LivingEntity cause) {
        this(value, lastMarkedTime, Optional.of(cause.getUUID()));
    }

    public Stigma(int value, LivingEntity cause) {
        this(value, cause.level().getGameTime(), Optional.of(cause.getUUID()));
    }

    public Optional<LivingEntity> getCause(ServerLevel level) {
        return cause.map(level::getEntity)
                .map(entity -> {
    ...
    private void lockHealth(LivingEntity livingEntity, LivingEntity cause) {
